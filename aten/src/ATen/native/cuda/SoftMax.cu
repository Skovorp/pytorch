...
struct SoftMaxForwardWithMulEpilogue {
  ...
  __device__ __forceinline__ OutT operator()(T input) const {
#ifdef PYTORCH_USE_FAST_SOFTMAX
...
template<template<typename, typename, typename> class Epilogue,
         template<typename, typename, typename> class EpilogueWithMul, bool is_log_softmax, bool uses_rocm>
        ...
        if (!half_to_float) {
          ...
          if (dim_size <= 1024 && dim_size*sizeof(scalar_t) <= 4096) {
            ...
            if (uses_rocm && !is_log_softmax) {
              ...
              cunn_SoftMaxForwardGmem<ILP, scalar_t, accscalar_t, scalar_t, Epilogue, EpilogueWithMul>
          ...
          if (dim_size <= 1024 && dim_size*sizeof(scalar_t) <= 4096) {
            ...
            if (uses_rocm && !is_log_softmax) {
              dim3 block(512);
              size_t smem_reduction_sz = block.x / C10_WARP_SIZE * sizeof(accscalar_t);
              cunn_SoftMaxForwardGmem<ILP, scalar_t, accscalar_t, accscalar_t, Epilogue, EpilogueWithMul>
                <<<grid, block, smem_reduction_sz, stream>>>(output_ptr, input_ptr, dim_size);
            } else {
              dim3 block = SoftMaxForward_getBlockSize(dim_size);
              size_t smem_reduction_sz = block.x / C10_WARP_SIZE * sizeof(accscalar_t);
